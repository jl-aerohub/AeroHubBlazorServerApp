...
namespace FileHandlingServer.Data
{
    ...
    public class AzureStorageHelper
    {
        ...
        public async Task<string> DownloadFile(string containerName, string sourceFilename, string destFileName)
        {
            ...
            try
            {
                var FileNameTest = Path.GetFileName(sourceFilename);

                BlobClient blob = container.GetBlobClient(FileNameTest);
        ...
        BlobContainerClient OpenContainer(string containerName)
        {
            try
            {
                string setting = configuration["AzureStorageConnectionString"];
